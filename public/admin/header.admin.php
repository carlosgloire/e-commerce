
<?php 

?>
    <header id="header" class="flex bg-white justify-between px-20 py-4 shadow-sm mb-1 border">
       
        <div class="flex gap-20">
        <p>
            <img src="images/gs_logo.png" alt="logo glory shop" width="30px" title="Acceuil">
        </p>
            <p class="text-black text-xl  font-semibold">ADMIN</p>
            
        </div>
        
         <div class="flex ">
         <form class="form-horizontal" action=" " method="post">
         <div class="flex gap-3">
                <input  class="rounded outline-none border-[1px] border-gray-500 bg-gray-100  px-3" type="search" id="search" name="search" placeholder="Rechercher un produit">
                <button type="submit" name="save" class="bg-blue-500 text-white px-2 mb-5 shadow-sm rounded text-center text-base hidden" >Submit</button>   
        </div>
         </form>
         </div>
     </header>

     <?php    
   
?>